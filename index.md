...
Implements the [Hit And Run Metropolis algorithm](http://www.bayesian-inference.com/mcmcharm). This kernel is only applicable to non-structural random choices with continueous values. It is very computationally efficient, updating all non-structural choices while requiring only one program evaluation. The option `scale` determines the largest possible jump the kernel will take (defaults to `1.0`), and the 'doScaleAdapt' flag determines whether the kernel automatically adapts its scale to achieve a target optimal acceptance ratio (defaults to `true`).
